var Context       = require('../media/js/context')
  ...
  , Definition    = require('../media/js/definition')
  , GameDefs      = require('../media/js/game_defs')
...
function Game(id, name, map_name) {
  ...
  this.defs = GameDefs(Definition, [this.context])
  this.clients = {}
}
...
Game.prototype.join = function(client_id, socket, ack_id) {
  ...
  this.send_all('update', this.context.create_update())

  setTimeout(function() {
  socket.emit('ack', {ack:ack_id, data:[
    null, {}, player.id, self.context.create_update(true) 
  ]})
  }, 0)
}
